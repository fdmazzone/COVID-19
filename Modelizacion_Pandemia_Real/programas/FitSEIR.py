...
def FitSEIR(Pais='Argentina',Data=None,Provincia=None,dpto=None,fecha=(), 
            R0_lim=(),R0_fijo=(),t_lim=(),t_lim_fijo=(),
            Rango_I0=((.1,1),),p=2.0,Metodo='shgo',save=None):
    """
    ##########################################################################
    ...
    if not Pais=='Argentina':
       ...
       ########### Leer datos país ############################################
        ...
        if not dpto==None:
            ...
            Data=readDataArg_dpto(Provincia, dpto,Data)
        else:
            Data=readDataArg(Provincia,Data)
            if not Provincia==None:
                Poblacion=load_fit_data(Provincia)
            else: 
                Poblacion=load_fit_data_world("Argentina")
    ...
    if Metodo=='dual_annealing':
        opt=scipy.optimize.dual_annealing(Error,rangos,args=(Ia,t,R0_fijo,t_lim_fijo,p))
        x_opt,error_opt=opt["x"],opt["fun"]
    elif Metodo=='shgo':
        opt=scipy.optimize.shgo(Error,rangos,args=(Ia,t,R0_fijo,t_lim_fijo,p))
        x_opt,error_opt=opt["x"],opt["fun"]
    elif Metodo=='dif_ev':
        opt=scipy.optimize.differential_evolution(Error,rangos,args=(Ia,t,R0_fijo,t_lim_fijo,p),workers=8)
        x_opt,error_opt=opt["x"],opt["fun"]
    elif Metodo=='brute':
        opt=scipy.optimize.brute(Error,rangos,args=(Ia,t,R0_fijo,t_lim_fijo,p),finish=None,full_output=True)
    ...
    if Provincia==None:
        ax.set_title(Pais+"("+fecha[1]+")",fontsize=26)
    elif not dpto==None:
        ax.set_title("Departamento "+dpto+"("+fecha[1]+")",fontsize=26)
    else:
        ax.set_title(Provincia+"("+fecha[1]+")",fontsize=26)
    ...
    for t in t_corte_opt:
        t_corte_g.append(t_ini+np.timedelta64(int(t),'D'))
...
def Error(x,*params):
    ...
    """
    Ia,t,R0_fijo,t_lim_fijo,p=params
    ...
    return np.sum(   np.abs(Ia-Sol[:,2]-Sol[:,3]-Sol[:,1])**p) 
...
########################### Provincias #######################################
def readDataArg(Provincia,Data):
    """
...
#################### Departamentos ###########################################
def readDataArg_dpto(Provincia, dpto_nam,Data):
    """
    Lee datos de la base descargada del ministerio de salud de la República
    Argentina.
...
def downloadARG():
    ...
    y los guarda en Data/Epidemic/Covid19Casos.zip
    ...
    folder='/home/fernando/fer/Investigacion/Trabajo_en_curso/COVID-19/Modelizacion_Pandemia_Real/programas/Data/Epidemic/'

    #old_name = folder+"Covid19Casos.csv"
    #new_name = folder+"Covid19Casos_viejo.csv"
    
        
    #os.remove('/home/fernando/fer/Investigacion/Trabajo_en_curso/COVID-19/Modelizacion_Pandemia_Real/programas/Data/Epidemic/Covid19Casos_viejo.csv')
    # Renaming the file
