...
def AnimacionARG(fecha,i):
    """
        Hace una animacion
        i=nro fotograma inicial
    """
    ...
    ###################### CARGA DATOS #####################################
    DataProv=readDataArg("Todas",fecha)
    ...
    for fecha_foto in I:
        ...
        #region centro 
        #ax.set_ylim(-35.3, -28.5);ax.set_xlim(-70.8, -57.6)
...
def readDataArg(Provincia,fecha):
    """
    ...
    for chunk in pd.read_csv('Data/Epidemic/Covid19Casos.csv',chunksize=chunk_size):
        ...
        if fecha:
            I1=DataC.fecha_apertura>=fecha[0]
            I2=DataC.fecha_apertura<=fecha[1]
            DataC=DataC[I1 & I2]
        dfs.append(DataC)
        batch_no+=1
    Data = pd.concat(dfs)
